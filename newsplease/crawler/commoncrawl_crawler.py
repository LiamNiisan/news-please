...
def __get_remote_index(warc_files_start_date, warc_files_end_date):
    ...
    with tempfile.NamedTemporaryFile() as temp:
        ...
        if warc_files_start_date or warc_files_end_date:
            ...
            for date in warc_dates:
                ...
                cmd += (
                    "aws s3 ls --recursive s3://commoncrawl/crawl-data/CC-NEWS/%s/%s/ >> %s && "
    ...
    lines = [p for p in lines if p.endswith(".warc.gz")]
...
def __start_commoncrawl_extractor(
    ...
    commoncrawl_extractor.extract_from_commoncrawl(
        ...
        filter_initial_keywords=filter_initial_keywords,
        filter_not_initial_keywords=filter_not_initial_keywords,
